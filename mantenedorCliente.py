...
def actualizar(cliente):
    ...
    try:
        with conexion.cursor() as cursor:
            consulta = "UPDATE cliente SET nombre = %s, apellido = %s, rut = %s, telefono = %s, correo = %s, direccion = %s, comuna = %s, region = %s WHERE rut = %s;"
                        #%s hace referencia a una variable string
                        #%d hace referencia a un int
                        #%f hace referencia a un float
                        #En la siguiente linea excecute nos permite ejecutar varias veces con distintos datos
...
def contar():
    ...
    try:
        with conexion.cursor() as cursor:
            cursor.execute("SELECT COUNT(id_cliente)FROM cliente")
            #usamos fetchall para traer todos los datos
            contCliente = cursor.fetchone()
            return contCliente[0]
    except (pymysql.err.OperationalError,pymysql.err.InternalError) as e:
        print("Ocurrio un error al consultar: ", e)
    conexion.close
    #-----------------
